...
export interface Store {
  ...
  name?: string;
  /** The underlying host mount (absent under `vite dev`). Needed by
   *  `useObjectUrl(mount, relPath)` to read images without copying bytes. */
  mount?: SandboxMount;
}
...
export async function openPrivateStore(sub = 'data'): Promise<Store> {
  ...
  return { root: join(m.path, sub), mode: m.mode === 'ro' ? 'ro' : 'rw', kind: 'settings', mount: m };
}
